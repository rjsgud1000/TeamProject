...
public class BoardReportController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (page == null) page = "1";
        if (reason == null || reason.trim().isEmpty()) reason = "기타";

        boolean alreadyReported = boardDAO.hasReportedPost(postId, loginMember.getMemberId());

        if (!alreadyReported) {
            boardDAO.reportPost(postId, loginMember.getMemberId(), reason);
        }
