...
<div class="detail-wrap detail-page">
	...
	<div class="card summary">
		...
		<div>
			...
			<div class="notice notice--spaced">
				<h2 class="section-title-inline">회원 상태 변경</h2>
				<c:choose>
					<c:when test="${member.role eq 'ADMIN'}">
						<p class="text-muted">관리자 계정은 상태를 변경할 수 없습니다.</p>
					</c:when>
					<c:otherwise>
						<form method="post" action="${contextPath}/member/admin/updateStatus.me" class="detail-status-form" onsubmit="return validateStatusForm();">
							...
							<label for="status" class="field-label">변경 상태</label>
							<select id="status" name="status" class="field-control" onchange="toggleSanctionFields();">
							...
							<div id="sanctionFields">
								<label for="sanctionReason" class="field-label">경고/제재 사유</label>
								<textarea id="sanctionReason" name="sanctionReason" rows="4" placeholder="경고 또는 제재 처리 시 사유를 입력하세요." class="field-control field-control--textarea"></textarea>
								<div id="sanctionEndAtWrap">
									<label for="sanctionEndAt" class="field-label">제재 종료 일시</label>
									<input type="datetime-local" id="sanctionEndAt" name="sanctionEndAt" class="field-control">
								</div>
							</div>
							<p class="helper-text">경고는 로그인 시 안내 후 자동 해제됩니다. 제재는 기간 종료 후 로그인 시 자동으로 활성화됩니다.</p>
	...
	<div class="card address-card">
		<h2 class="address-title">주소 정보</h2>
